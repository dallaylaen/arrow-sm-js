...
(function(window){
    ...
    function ArrowSM(dsl = {}) {
        ...
        this.start = function(initial) {
            ...
            const decide = function( arg, target ) {
                let next;
                if (me.decide)
                    next = me.decide.apply(target, [arg, state]);
                if (next === undefined) {
                    const decide = me.states[state].decide;
                    if (decide !== undefined)
                        next = decide.apply(target, [arg, state]);
                }
                if (Array.isArray(next)) {
                    setState.apply(target, [next[0], arg]);
                    return next[1];
                } else {
                    setState.apply(target, [next, arg]);
                }
            };
                ...
                return new Promise( done => {
                    ...
                    try {
                        ...
                        while( queue.length ) {
                            ...
                            task[0]( decide( task[1], task[2] ));
                        }
                    } finally {
                        // TODO reject leftover promises
                        queue = [];
                        lock = 0;
                    }
                });
